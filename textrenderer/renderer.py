...
class Renderer(object):
    def __init__(self, corpus, fonts, bgs, cfg, width=256, height=32,
                 clip_max_chars=False, debug=True, gpu=False, strict=False):
        self.cnt = 0
        self.corpus = corpus
        self.fonts = fonts
        self.bgs = bgs
    ...
    def gen_img(self, img_index):
        ...
        if apply(self.cfg.random_direction):
            if random.uniform(0, 1) > self.cfg.random_direction.fraction:
                dir = "horizon"
            else:
                dir = 'vertical'
        word_img, text_box_pnts, word_color = self.draw_text_on_bg(word, font, bg, dir)
    ...
    def draw_text_on_bg(self, word, font, bg, dir):
        ...
        if dir == "vertical":
            ...
            if self.is_bgr():
                ...
                word_color = self.get_gray_word_color(v_bg, text_x, text_y, word_height, word_width)
            ...
            if apply(self.cfg.random_space):
                ...
                if apply(self.cfg.text_border):
                    for item in word:
                        ...
                        border_color = self.draw_border_text(draw, item, startx, starty, font, word_color,border_color)
                        # img = cv2.cvtColor(np.asarray(pil_img), cv2.COLOR_RGB2BGR)
                        # cv2.imshow("1",img)
                        # cv2.waitKey(0)
                        starty += word_height
            ...
            if apply(self.cfg.random_space):
                text_x, text_y, word_width, word_height = self.draw_text_with_random_space(draw, font, word,
                                                                                           word_color,
                                                                                           bg_width, bg_height,dir)
    ...
    def draw_text_with_random_space(self, draw, font,
        word, word_color, bg_width, bg_height, dir):
        ...
        if dir == 'horizon':
            ...
            for c in word:
                ...
                height += size[1]
                # set max char width as word width
                if size[0] > width:
                    width = size[0]
    ...
    def draw_text_wrapper(self, draw, text, x, y, font, text_color,border_color = None):
        """
    ...
    def draw_border_text(self, draw, text, x, y, font, text_color,border_color = None):
        """
        :param x/y: 应该是移除了 offset 的
        """
